...
constexpr char kTag[] = "official_settings";
...
void Settings::SetString(const std::string &key, const std::string &value) {
  if (!read_write_) {
    ESP_LOGW(kTag, "namespace %s is not open for writing", ns_.c_str());
    return;
  }
...
int32_t Settings::GetInt(const std::string &key, int32_t default_value) const {
  if (nvs_handle_ == 0) {
    return default_value;
  }
