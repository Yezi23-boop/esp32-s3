...
static void weather_service_store_result(const weather_http_result_t *result)
{
    if (result == NULL) {
        return;
    }
    if (s_weather_mutex == NULL) {
        ...
        if (s_weather_mutex == NULL) {
            ESP_LOGE(TAG, "Weather snapshot mutex allocation failed");
            return;
        }
    }
    ...
    {
        s_weather_info.temp = result->temperature;
        strncpy(s_weather_info.weather_text, result->text,
                sizeof(s_weather_info.weather_text) - 1);
        s_weather_info.weather_text[sizeof(s_weather_info.weather_text) - 1] = '\0';
        const char *icon = get_weather_icon_by_code(result->code);
        ...
        ESP_LOGI(TAG, "Weather service state updated: temp=%d, text=%s, icon=%s",
                 result->temperature, result->text, icon);
    }
}
    ...
    while (1)
    {
        ...
        if (net_state == NETWORK_MANAGER_STATE_CONNECTED)
        {
            if (seconds_since_last_fetch >= refresh_interval_seconds)
            {
                ...
                weather_http_result_t result = {0};
                esp_err_t err = weather_http_client_fetch(&result);
                if (err == ESP_OK) {
                    weather_service_store_result(&result);
